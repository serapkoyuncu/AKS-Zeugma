...
bool isValidVoltage() {
  for (int i = 0; i < numCells; i++) {
    if (bmsData.cellVoltages[i] >= 4800) {
      return false;  // Geçersiz voltaj bulundu, relay açılmayacak
    }
  }
  return true;  // Tüm voltajlar geçerli
}
...
void setup() {
 ...
 // DDRD |= (1 << PD6);  // PD7'yi (38. pin) çıkış olarak ayarla
  ...
  DDRD |= (1 << PD4);  // PD7'yi (38. pin) çıkış olarak ayarla
...
void loop() {
  ...
  Wire.requestFrom(8, 5);              // request 2 bytes from slave device #8
  while (Wire.available() < 5) {
    if (millis() - startTime >= 200) {
      Serial.println("I2C: Timeout: Data not received in time.");
      break;
    }
  }
  ...
  // PF0 pininin durumunu kontrol et (PIN: Port Input)
  if (PINF & (1 << PF0)) {
    // PF0 HIGH durumda, bir şey yapma
  } else {
    // PF0 LOW durumda, PD6 pinini deaktif (LOW) yap
    PORTD &= ~(1 << PD6);
  }
  ...
  geciciDizi1[20] = 0;  // random(0, 120);
  data.arac_hizi = 0;   // random(0, 120);
...
int enBuyukBaytiBul(int dizi[20]) {
  int enBuyuk = dizi[0];  // İlk baytı en büyük olarak kabul et

  for (int i = 1; i < 20; i++) {  // Dizinin geri kalanını kontrol et
    if (dizi[i] > enBuyuk) {
      enBuyuk = dizi[i];  // Daha büyük bir bayt bulunduğunda güncelle
    }
  }
  return enBuyuk;  // En büyük baytı geri döndür
}

void kontrolRole() {
  if (isValidVoltage() && enBuyukBaytiBul(bmsData.cellVoltages) >= 4200) {  // DUZENLENECEK
    PORTD |= (1 << PD4);
  }
}
...
bool requestData(uint8_t requestId) {
  ...
  while (millis() - startTime < 500) {  // BURASI 3000 ms OLACAK
  ...
  Serial.println("Timeout waiting for CAN BUS");
  return false;
}
...
void updateValues() {
  ...
  if (sarjDurumuFlag) {  // DÜZENLENECEK
