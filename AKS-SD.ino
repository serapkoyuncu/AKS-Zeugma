...
bool requestDataForPreCharge() {
  // Veri gönderme kısmı
  Transmitted_Data.data[0] = 88;
  mcp2515.sendMessage(&Transmitted_Data);
  Serial.println("NANO'YA Veri gönderildi.");
  ...
  while (millis() - startTime < 3000) {  // 3 saniye timeout
    if (mcp2515.readMessage(&Received_Data) == MCP2515::ERROR_OK) {
      if (Received_Data.can_id == 0x388 && Received_Data.data[0] == 212) {
        Serial.print("*******************************");
        processRDPreCharge();
        return true;
      }
...
bool requestData(uint8_t requestId) {
  ...
  Serial.println("STM32'YE Veri gönderildi.");
...
void veriKaydetSD() {
  if (veriDosyasi) {
    ...
    if (data.kalanEnerjiMiktari == 0 && data.toplamVoltaj == 0 && bmsData.maxTemp == 0) {
      Serial.println("tüm değerler sıfır");
      return;
    }
