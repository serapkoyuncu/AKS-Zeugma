...
void loop() {
  ...
  while (e22ttl.available() > 1) {
    ...
    for (int i = 0; i < 20; i++) {
      veriler2obj.hucreVoltajlari[i] = float(receivedData.dizi[i])/10.0;
    }
    veriler2obj.arac_hizi = receivedData.dizi[20];
    veriler2obj.toplamVoltaj = receivedData.dizi[21];
    veriler2obj.kalanEnerjiMiktari = receivedData.dizi[22]; // IPTAL
    veriler2obj.sicaklik = receivedData.dizi[23];
    veriler2obj.kalanEnerjiMiktari = (receivedData.dizi[24] << 8) | receivedData.dizi[25];
    ...
    // Araç hızı kontrolü
    if (veriler2obj.arac_hizi < 1 || veriler2obj.arac_hizi > 0) {
      validData = false;
    }

    // Toplam voltaj kontrolü
    if (veriler2obj.toplamVoltaj < 1 || veriler2obj.toplamVoltaj > 10) {
      validData = false;
    }

    // Kalan enerji miktarı kontrolü
    if (veriler2obj.kalanEnerjiMiktari < 1 || veriler2obj.kalanEnerjiMiktari > 200) {
      validData = false;
    }

    // Hücre voltajlarını kontrol et
    for (int i = 0; i < 20; i++) {
      if (veriler2obj.hucreVoltajlari[i] < 1 || veriler2obj.hucreVoltajlari[i] > 2) {
        validData = false;
        break;
      }
    }
    ...
    if (veriler2obj.sicaklik < 1 || veriler2obj.sicaklik > ) {
      validData = false;
    }*/
